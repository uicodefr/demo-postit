...
export class RestClientService {
  public constructor(private httpClient: HttpClient, private globalInfoService: GlobalInfoService) {}
  ...
  public get<T>(url: string, paramData?: any): Observable<T> {
    ...
    const obsResponse = this.httpClient.get(url, {
      observe: 'response',
      responseType: 'json',
      params: new HttpParams({ fromObject: paramData })
    });
  ...
  public delete<T>(url: string): Observable<T> {
    this.globalInfoService.notifLoader(true);
    const obsResponse = this.httpClient.delete(url, { observe: 'response', responseType: 'json' });
    return this.addMapperAndCatcher<T>(obsResponse, url);
  }
  ...
  public getBlob(url: string, paramData?: any): Observable<{} | Blob> {
    this.globalInfoService.notifLoader(true);
    const obsResponse = this.httpClient
      .get(url, { observe: 'body', responseType: 'blob' })
  ...
  private handleError(url: string) {
    ...
    return function(err) {
      ...
      if (err instanceof HttpErrorResponse) {
        if (err.status === 401) {
          globalInfoService.showAlert(
            AlertType.WARNING,
            $localize`:@@global.accessUnauthorized:Access Unauthorized. Please sign in.`
          );
          return NEVER;
        } else if (err.status === 403) {
          globalInfoService.showAlert(AlertType.DANGER, $localize`:@@global.accessForbidden:Access Forbidden !`);
          return NEVER;
        }
